...
do
    ...
    output_file="cifar_mod_$beta_formatted"

    # Uncomment the following line to run the actual command
    CUDA_VISIBLE_DEVICES=0 python3 -m advbench.scripts.train --dataset CIFAR10 --algorithm CVaR_Modified_SGD --output_dir rep_three/$output_file --cvar_sgd_beta $beta --cvar_sgd_t_step_size $t_step_size  --evaluators Clean PGD Augmented
done